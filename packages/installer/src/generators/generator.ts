...
import { deleteFile } from '../utils/mem-fs-delete-file'

const debug = require('debug')('pliny:generator')
...
> extends EventEmitter {
  ...
  async write(): Promise<void> {
    ...
    for (const filePath of paths) {
      try {
        ...
        // Using the internal _copySingle function avoids glob [slug].js problem and just copies the file as intended
        this.fs._copySingle(this.sourcePath(filePath), this.destinationPath(pathSuffix), {
          process: (input) =>
        ...
        if (templatedPathSuffix !== pathSuffix) {
          // Use _copySingle and deleteFile to mock fs.move without glob issues
          this.fs._copySingle(
            this.destinationPath(pathSuffix),
            this.destinationPath(templatedPathSuffix)
          )
          deleteFile(this.destinationPath(pathSuffix), this.store)
        }
      } catch (error) {
  ...
  async preWrite(): Promise<void> {
    // expose preWrite hook, no default implementation
  }
