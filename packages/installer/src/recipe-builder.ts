...
import * as RequireArgsExecutor from './executors/require-args'
...
export interface IRecipeBuilder {
  ...
  requireArgs(step: Omit<RequireArgsExecutor.Config, 'stepType'>): IRecipeBuilder
...
export function RecipeBuilder(): IRecipeBuilder {
  ...
  return {
    ...
    requireArgs(step: Omit<RequireArgsExecutor.Config, 'stepType'>) {
      steps.push({
        stepType: RequireArgsExecutor.type,
        ...step,
      })
      return this
    },
