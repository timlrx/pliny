...
export type TransformConfigCallback = (config: j.ObjectExpression) => j.ObjectExpression

export function transformConfig(program: Program, transform: TransformConfigCallback): Program {
  ...
  if (moduleExportsExpressions.length === 0) {
    ...
    if (config) {
      ...
      console.warn(
        "The configuration couldn't be found, but there is a 'module.exports' inside config file"
      )
    }
  } else {
    console.warn("There are multiple 'module.exports' inside config file")
  }
